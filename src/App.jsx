  ...
  return (
    <div style={{ background: "#0a0818", minHeight: "100vh", fontFamily: "'Inter', system-ui, sans-serif" }}>
      ...
      {!isVendors && g && !isLoading && (
        <div style={{ background: "#0d0b20", borderBottom: "1px solid #1a1640", padding: "10px 24px" }}>
          <div style={{ maxWidth: 960, margin: "0 auto", display: "flex", gap: 8, alignItems: "center", flexWrap: "wrap" }}>
            ...
            <div style={{ flex: 1 }} />
            {/* Sort toggle */}
            <div style={{ display: "flex", background: "#100e28", border: "1px solid #1a1640", borderRadius: 8, overflow: "hidden" }}>
              {[["month","By Month"],["status","By Status"]].map(([val, label]) => (
                <button key={val} onClick={() => setSortBy(val)} style={{
                  padding: "5px 12px", background: sortBy === val ? "#1e1c40" : "transparent",
                  border: "none", color: sortBy === val ? "#a0a0e0" : "#3a3860",
                  fontSize: 11, fontWeight: sortBy === val ? 700 : 500, cursor: "pointer"
                }}>{label}</button>
              ))}
            </div>
          </div>
      ...
      {/* ── MONTH FILTER + ADD ── */}
      <div style={{ padding: "12px 24px", display: "flex", gap: 8, flexWrap: "wrap", alignItems: "center", borderBottom: "1px solid #1a1640", maxWidth: 960, margin: "0 auto", width: "100%" }}>
        {/* For tasks: month pills */}
        {!isVendors && (
          <div style={{ display: "flex", gap: 6, flex: 1, flexWrap: "wrap" }}>
            {["All", ...MONTHS.filter(m => usedMonths.includes(m))].map(m => {
              const on = filter === m;
              return (
        ...
        {/* For vendors: category pills */}
        {isVendors && (
          <div style={{ display: "flex", gap: 6, flex: 1, flexWrap: "wrap" }}>
            {["All", ...VENDOR_CATEGORIES.filter(c => usedCats.includes(c))].map(c => {
        ...
        <button onClick={() => isVendors ? startAddVendor() : startAddTask()} className="btn-primary"
          style={{ padding: "8px 18px", borderRadius: 8, background: isVendors ? "#e0a040" : tabAcc,
            ...
            letterSpacing: 0.5, boxShadow: `0 4px 16px ${isVendors ? "#e0a04040" : tabAcc+"40"}` }}>
